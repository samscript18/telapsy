	...
	return (
		<div className="shell py-12">
			<div data-reveal className="max-w-2xl">
				...
				<h1 className="mt-3 text-4xl font-extralight tracking-[-0.04em] text-[var(--ink)] md:text-6xl">Discover your next essential.</h1>
				<p className="mt-4 text-base font-light leading-relaxed text-[var(--muted)]">Forty masterfully crafted pieces across 4 signature departments. Curated for timeless utility and design excellence.</p>
			</div>
			...
			<div data-reveal style={{ "--reveal-delay": "100ms" } as React.CSSProperties} className="mt-10 flex flex-col gap-4 border-y border-[var(--line)] py-6 md:flex-row md:items-center md:justify-between">
				...
				<label className="relative block md:w-80">
					...
					/>
					{search && (
						<button onClick={() => setSearch("")} className="absolute right-3 top-3 text-[var(--faint)] hover:text-[var(--ink)]" aria-label="Clear search">
							<X size={16} />
						</button>
					)}
				</label>
			...
			{!isLoading && (data?.count ?? 0) > PAGE_SIZE && (
				<nav className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-[var(--line)] pt-7 sm:flex-row" aria-label="Product pagination">
					<p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--faint)]">
						Page {page} of {Math.ceil((data?.count ?? 0) / PAGE_SIZE)}
					</p>
					<div className="flex items-center gap-2">
						<button disabled={page === 1} onClick={() => updatePage(page - 1)} className="pagination-button">
							<ChevronLeft size={15} />
							Previous
						</button>
						{Array.from({ length: Math.ceil((data?.count ?? 0) / PAGE_SIZE) }, (_, index) => index + 1).map((number) => (
							<button key={number} onClick={() => updatePage(number)} aria-current={number === page ? "page" : undefined} className={`pagination-number ${number === page ? "is-active" : ""}`}>
								{number}
							</button>
						))}
						<button disabled={page >= Math.ceil((data?.count ?? 0) / PAGE_SIZE)} onClick={() => updatePage(page + 1)} className="pagination-button">
							Next
