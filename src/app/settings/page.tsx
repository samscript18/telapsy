...
function SettingsExperience({ user }: { user: SessionUser }) {
  ...
  const { data: sessionData, isLoading: sessionsLoading, refetch: refetchSessions } = useQuery<{ sessions: AccountSession[] }>({
  ...
  return (
    <div className="account-page mx-auto max-w-6xl">
      ...
      <section className="settings-profile-summary" data-reveal aria-labelledby="settings-name">
        ...
        <Link href="/profile">View full profile <ArrowUpRight size={15} /></Link>
      </section>

      <div className="settings-content-grid">
        <form id="profile" className="account-panel scroll-mt-24" onSubmit={saveProfile}>
          <PanelHeader icon={<PencilLine size={17} />} kicker="Profile details" title="Present a clear identity" description="These details identify you throughout your private shopping account." />
          <fieldset className="account-fieldset"><legend>Personal identity</legend><p>Keep the name attached to your account and orders accurate.</p><label><span>Full name</span><input name="name" defaultValue={user.name} required minLength={2} /></label><label><span><Mail size={13} />Email address</span><input value={user.email} disabled /><small>Your login email cannot be changed here.</small></label></fieldset>
          {profileStatus && <p className="account-form-status" role="status"><Check size={14} />{profileStatus}</p>}
          <footer className="account-form-footer"><span>Saved to your Telapsy account.</span><button disabled={savingProfile}>{savingProfile ? "Saving profile…" : "Save profile"}</button></footer>
        </form>

        <section id="notifications" className="account-panel scroll-mt-24">
          <PanelHeader icon={<BellRing size={17} />} kicker="Notification delivery" title="Choose what deserves attention" description="Control the account updates and product messages you want to receive." />
          <div className="settings-preference-list"><SettingRow icon={<PackageCheck size={17} />} title="Order updates" text="Confirmations and important status changes for your orders." checked={preferences.orderUpdates} onToggle={() => toggle("orderUpdates")} /><SettingRow icon={<Mail size={17} />} title="Collection notes" text="Occasional news when the considered collection changes." checked={preferences.productNews} onToggle={() => toggle("productNews")} /><SettingRow icon={<MonitorCog size={17} />} title="Compact dashboard" text="A denser account overview on supported screens." checked={preferences.compactDashboard} onToggle={() => toggle("compactDashboard")} /></div>
          {preferenceStatus && <p className="account-form-status" role="status"><Check size={14} />{preferenceStatus}</p>}
      ...
      <section id="security" className="account-panel account-security-panel scroll-mt-24">
        ...
        {user.authProvider === "google" ? (
          <div className="security-method"><span><ShieldCheck size={17} /></span><div><small>Password management</small><strong>Managed by Google</strong><p>Telapsy never receives your Google password. Change it from your Google Account security settings.</p></div></div>
        ) : (
          <form onSubmit={changePassword} className="mt-6 grid gap-4">
            <label className="grid gap-2 text-xs text-[var(--muted)]"><span>Current password</span><input className="field" name="currentPassword" type="password" autoComplete="current-password" required /></label>
            <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-xs text-[var(--muted)]"><span>New password</span><input className="field" name="newPassword" type="password" autoComplete="new-password" minLength={12} required /></label><label className="grid gap-2 text-xs text-[var(--muted)]"><span>Confirm new password</span><input className="field" name="confirmPassword" type="password" autoComplete="new-password" minLength={12} required /></label></div>
            <div className="mt-2 flex flex-col items-start justify-between gap-4 rounded-2xl border border-[rgba(110,231,183,.18)] bg-[rgba(110,231,183,.04)] p-4 sm:flex-row sm:items-center"><p className="text-xs leading-5 text-[#91d8b5]">Use at least 12 characters with uppercase, lowercase, a number, and a special character.</p><button disabled={changingPassword} className="btn btn-primary shrink-0 rounded-full px-6 py-3 text-xs">{changingPassword ? "Changing password…" : "Change password"}</button></div>
          </form>
        )}
        {securityStatus && <p className="account-form-status" role="status">{securityStatus}</p>}
      </section>

      <section className="account-panel account-security-panel" aria-labelledby="active-sessions-title">
        <PanelHeader icon={<MonitorCog size={17} />} kicker="Account access" title="Active sessions" description="Review the devices currently authenticated to your Telapsy account." id="active-sessions-title" />
        {sessionsLoading ? <div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="h-56 animate-pulse rounded-2xl bg-white/[.025]" /><div className="h-56 animate-pulse rounded-2xl bg-white/[.025]" /></div> : (
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {sessionData?.sessions.map((session) => <SessionCard key={session.id} session={session} loading={revokingSession === session.id} onRevoke={() => revokeSession(session)} />)}
          </div>
        )}
      ...
      <section id="privacy" className="account-panel account-security-panel scroll-mt-24" aria-labelledby="delete-account-title">
        ...
        <form onSubmit={deleteAccount} className="mt-6 grid gap-4 rounded-2xl border border-[#df8585]/20 bg-[#df8585]/[.035] p-5 sm:p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-xs text-[var(--muted)]"><span>Confirmation</span><input className="field" name="confirmation" placeholder="Type DELETE" autoComplete="off" required /></label>{user.authProvider !== "google" && <label className="grid gap-2 text-xs text-[var(--muted)]"><span>Current password</span><input className="field" name="password" type="password" autoComplete="current-password" required /></label>}</div>
          {deleteStatus && <p role="alert" className="text-xs text-[#f2a6a6]">{deleteStatus}</p>}
...
function SessionCard({ session, loading, onRevoke }: { session: AccountSession; loading: boolean; onRevoke: () => void }) {
  return <article className={`overflow-hidden rounded-2xl border ${session.current ? "border-[rgba(232,185,106,.28)] bg-[linear-gradient(145deg,rgba(232,185,106,.08),rgba(255,255,255,.018))]" : "border-[var(--line)] bg-white/[.018]"}`}><header className="flex items-center gap-3 border-b border-[var(--line)] p-5"><span className="grid size-11 place-items-center rounded-xl bg-[rgba(232,185,106,.1)] text-[var(--accent)]">{session.device.includes("iPhone") || session.device.includes("Android") ? <Smartphone size={19} /> : <MonitorCog size={19} />}</span><div className="min-w-0"><strong className="block text-sm font-medium text-[var(--ink)]">{session.device}</strong><span className="mt-1 block truncate text-xs text-[var(--faint)]">{session.browser} · {session.operatingSystem}</span></div>{session.current && <span className="ml-auto rounded-full border border-[rgba(110,231,183,.2)] bg-[rgba(110,231,183,.07)] px-2.5 py-1 text-[9px] uppercase tracking-[.12em] text-[#a7f3d0]">This device</span>}</header><dl className="grid grid-cols-2"><div className="border-b border-r border-[var(--line)] p-4"><dt className="flex items-center gap-1.5 text-[9px] uppercase tracking-[.12em] text-[var(--faint)]"><Clock3 size={12} />Last active</dt><dd className="mt-2 text-xs text-[var(--muted)]">{formatDate(session.lastActiveAt)}</dd></div><div className="border-b border-[var(--line)] p-4"><dt className="text-[9px] uppercase tracking-[.12em] text-[var(--faint)]">Expires</dt><dd className="mt-2 text-xs text-[var(--muted)]">{formatDate(session.expiresAt)}</dd></div></dl><footer className="flex items-center justify-between gap-3 p-4"><span className="text-[10px] leading-4 text-[var(--faint)]">{session.current ? "Signing out ends access on this device." : "Remove access if you do not recognize it."}</span><button type="button" onClick={onRevoke} disabled={loading} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-[#df8585]/25 px-4 py-2.5 text-[10px] text-[#e9a0a0] transition hover:bg-[#df8585]/08 disabled:opacity-50"><LogOut size={13} />{loading ? "Revoking…" : session.current ? "Sign out this device" : "Revoke access"}</button></footer></article>;
}

function formatDate(value: string) { return new Date(value).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }); }
function PanelHeader({ icon, kicker, title, description, id, tone }: { icon: React.ReactNode; kicker: string; title: string; description: string; id?: string; tone?: "danger" }) { return <header className={`account-panel-header ${tone === "danger" ? "[&>span]:!bg-[#df8585]/10 [&>span]:!text-[#f2a6a6]" : ""}`}><span>{icon}</span><div><small className={tone === "danger" ? "!text-[#f2a6a6]" : ""}>{kicker}</small><h3 id={id}>{title}</h3><p>{description}</p></div></header>; }
