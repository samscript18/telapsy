...
import { GOOGLE_NONCE_COOKIE, GOOGLE_RETURN_COOKIE, GOOGLE_STATE_COOKIE, GOOGLE_VERIFIER_COOKIE, googleConfigured, googleRedirectUri, verifyGoogleIdToken } from "@/lib/google-auth";
import { User } from "@/models/User";

function signinError(code: string) {
	const response = NextResponse.redirect(new URL(`/signin?error=${code}`, process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"));
...
export async function GET(request: NextRequest) {
	if (!googleConfigured()) return signinError("google_not_configured");
	...
	if (!code || !state || !expectedState || state !== expectedState || !nonce || !verifier) {
		return signinError("google_state");
	}
	...
	try {
		...
		if (!user) {
			...
			if (emailOwner && !emailOwner.googleSub) return signinError("google_link_required");
		...
		return signinError("google_failed");
	}
}
