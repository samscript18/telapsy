...
export async function POST(request: Request) {
  ...
  await connectDb(); const user = await User.findOne({ email: parsed.data.email });
  ...
  response.cookies.set(SESSION_COOKIE, await createSessionToken(String(user._id)), sessionCookieOptions); return response;
}
