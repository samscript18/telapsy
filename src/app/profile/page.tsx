...
import { AtSign, CalendarDays, Camera, Check, CheckCircle2, ImagePlus, KeyRound, Mail, PencilLine, ShieldCheck, UserRound, WalletCards } from "lucide-react";
				...
				<div className="account-page mx-auto max-w-6xl">
					...
					<div className="profile-content-grid">
						<section className="account-panel" aria-labelledby="profile-details-title">
							<PanelHeader
								...
								description="Identity and account information securely connected to Telapsy."
						...
						<section id="profile-photo" className="account-panel scroll-mt-24" aria-labelledby="profile-photo-title">
							<PanelHeader
								...
								title="Keep your account recognizable"
								description="Choose a clear image that will identify your Telapsy account."
							...
							<div className="mt-5 flex items-center gap-4 rounded-2xl border border-[var(--line)] bg-black/15 p-4">
								...
								<div className="min-w-0">
									<strong className="block text-sm font-medium text-[var(--ink)]">Current account image</strong>
									<span className="mt-1 block truncate text-xs text-[var(--faint)]">{imageName || "Shown across your profile and account."}</span>
								</div>
							</div>
							<label className="mt-4 grid min-h-40 cursor-pointer place-items-center rounded-2xl border border-dashed border-[rgba(232,185,106,.24)] bg-[rgba(232,185,106,.035)] p-6 text-center transition hover:border-[rgba(232,185,106,.48)] hover:bg-[rgba(232,185,106,.06)]">
								...
								<span>
									<ImagePlus className="mx-auto text-[var(--accent)]" size={24} />
									<strong className="mt-3 block text-sm font-medium text-[var(--ink)]">Choose an image</strong>
									<small className="mt-1 block text-[10px] leading-5 text-[var(--faint)]">JPEG, PNG or WebP · Maximum 2 MB</small>
								</span>
							</label>
							{imageStatus && (
								<p role="status" className="account-form-status">
									<Check size={14} />
									{imageStatus}
								</p>
							)}
							<button
								...
								className="btn btn-primary mt-4 w-full rounded-full py-3 text-xs disabled:cursor-not-allowed disabled:opacity-40"
							>
								{savingImage ? "Uploading..." : "Update profile image"}
							</button>
						</section>
					</div>
					...
					<form onSubmit={save} className="account-panel mt-5" aria-labelledby="edit-profile-title">
						<PanelHeader
							...
							description="This name appears throughout your account and order history."
							id="edit-profile-title"
						/>
						<fieldset className="account-fieldset">
							<legend>Personal identity</legend>
							<p>Choose the full name you want Telapsy to use.</p>
							<label>
								<span>Full name</span>
								<input key={user.name} name="name" defaultValue={user.name} required minLength={2} autoComplete="name" />
							</label>
							<label>
								<span>Email address</span>
								<input value={user.email} disabled />
								<small>Your login email is locked for account security.</small>
							</label>
						</fieldset>
						{status && (
							<p role="status" className="account-form-status">
						...
						<footer className="account-form-footer">
							<span>Changes apply across your Telapsy account.</span>
							<button disabled={saving}>{saving ? "Saving profile…" : "Save profile"}</button>
						</footer>
					</form>
				</div>
			)}
...
function Avatar({ name, image, className }: { name: string; image?: string | null; className: string }) {
	return image ? (
		<span className={`relative block ${className}`}>
			<Image src={image} alt={`${name} profile`} fill unoptimized className="object-cover" sizes="96px" />
		</span>
	) : (
		<span className={className}>{name.slice(0, 1).toUpperCase()}</span>
	);
}

function PanelHeader({ icon, kicker, title, description, id }: { icon: React.ReactNode; kicker: string; title: string; description: string; id: string }) {
	return (
		<header className="account-panel-header">
			<span>{icon}</span>
			<div>
				<small className="text-[15px]!">{kicker}</small>
