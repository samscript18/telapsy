...
export default async function DashboardProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <AccountShell title={product.name} eyebrow={product.category}><ProductDetails product={product}/><section className="mt-24"><p className="eyebrow">Keep looking</p><h2 className="display mt-3 text-5xl">You may also like</h2><div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-6">{related.map((item)=><ProductCard key={item.slug} product={item} authenticated/>)}</div></section></AccountShell>;
}
