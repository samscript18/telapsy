...
export function AuthForm({ mode, next }: { mode: "signup" | "signin"; next?: string }) {
	...
	async function submit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			const response = await fetch(`/api/auth/${mode}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.get("name"), email: form.get("email"), password }) });
		...
		<form onSubmit={submit} className="mt-8 grid gap-5">
			...
			<label className="grid gap-1.5 text-xs font-mono text-[var(--faint)]">
				...
				<span className="relative">
					<input
						className="field pr-12 text-xs text-[var(--ink)] font-sans"
					...
					{signup && (
						<button
							...
							className="absolute right-3 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-full text-[var(--faint)] hover:text-[var(--accent)]"
			...
			{signup && (
				<div className="password-requirements">
					...
					{requirements.map((requirement, index) => {
						...
						return (
							<div key={requirement.label} className={met ? "is-met" : ""}>
								{met ? <CheckCircle2 size={18} /> : <Circle size={18} />}
			...
			{!signup && (
				<Link href="/forgot-password" className="-mt-2 text-right text-xs font-mono text-[var(--accent)]! underline hover:text-[var(--accent-bright)]">
					Forgot password?
				</Link>
			)}
			{signup && (
			...
			<button disabled={loading} className="btn btn-primary w-full rounded-full py-3.5 text-sm font-medium disabled:opacity-60">
				{loading ? (signup ? "Creating account…" : "Logging in…") : signup ? "Sign up" : "Login"}
			</button>
			<p className="text-center text-xs font-light text-[var(--muted)]">
				{signup ? "Already have an account?" : "New to Telapsy?"}{" "}
				<Link className="font-mono text-[var(--accent)]! hover:text-[var(--accent-bright)] hover:underline" href={`${signup ? "/signin" : "/signup"}${next ? `?next=${encodeURIComponent(next)}` : ""}`}>
