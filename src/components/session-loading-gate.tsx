...
export function SessionLoadingGate() {
	...
	const clipId = `telapsy-word-${identifier}`;
	const gradientId = `telapsy-liquid-${identifier}`;
	return (
		<main className="session-gate" aria-busy="true">
			...
			<svg viewBox="0 0 620 150" className="session-gate-wordmark" aria-hidden="true">
				<defs>
					<clipPath id={clipId}>
						<text x="310" y="104" textAnchor="middle" className="session-gate-text">
							TELAPSY
						</text>
					</clipPath>
				...
				<text x="310" y="104" textAnchor="middle" className="session-gate-text session-gate-outline">
					TELAPSY
				</text>
				<g clipPath={`url(#${clipId})`}>
					<rect width="620" height="150" fill={`url(#${gradientId})`} />
			...
			<div className="session-gate-status">
				<span />
				Securing your Telapsy session
			</div>
			<p className="sr-only" role="status">
				Telapsy is restoring your authenticated session.
			</p>
		</main>
	);
}
